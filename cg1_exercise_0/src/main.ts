...
function createGUI(settings: Settings): dat.GUI {
  ...
  gui.add(settings, 'scale', 0, 1, 0.01).name('Scale');
...
function callback(changed: utils.KeyValuePair<Settings>) {
  if (changed.key == "name") {
    ...
    if (changed.value == true)
      mesh.material = new THREE.MeshPhongMaterial({ color: 0x781726 });
    else 
      mesh.material = new THREE.MeshPhongMaterial({ color: 0x989796 });
  }
}
...
function main(){
  ...
  var rendererDiv = createWindow("area1");
  ...
  //Adds a keyboard input, so that a random background color is assigned to the scene when the spacebar is pressed.
